	...
	<view class="content">
		<view class="forms">
			...
			<view class="person-pic">
				...
				<view class="pic-list">
					<view class="uploadPic" @click="isClick&&zmPic()" v-if="IDCardPositive!=''">
						<image :src="IDCardPositive" class="newPic" mode="aspectFill"></image>
					</view>
					<view class="uploadPic" @click="isClick&&zmPic()" v-else>
						<image src="../../static/photo.png" class="oldPic" mode="aspectFill"></image>
						<text>上传正面照片</text>
					</view>
					
					<view class="uploadPic" @click="isClick&&fmPic()" v-if="IDCardNegative!=''">
						<image :src="IDCardNegative" class="newPic" mode="aspectFill"></image>
					</view>
					<view class="uploadPic" @click="isClick&&fmPic()" v-else>
						<image src="../../static/photo.png" class="oldPic" mode="aspectFill"></image>
						<text>上传反面照片</text>
					</view>
					
					<view class="uploadPic" @click="isClick&&ewm()" v-if="QRCode!=''">
						<image :src="QRCode" class="newPic" mode="aspectFill"></image>
					</view>
					<view class="uploadPic" @click="isClick&&ewm()" v-else>
						<image src="../../static/photo.png" class="oldPic" mode="aspectFill"></image>
						<text>上传二维码</text>
					</view>
				</view>
			...
			<view class="person-item">
				<label>手机号</label>
				<input type="text" placeholder="填写手机号" v-model="PhoneNumber2"/>
			</view>
			<view class="person-item">
				<label>地址</label>
...
<script>
	...
	export default {
		...
		onLoad() {
			...
			this.states=uni.getStorageSync('states');
		},
		onReady(){
		...
		methods: {
			...
			showInfo(){
				...
				uni.request({
					...
					success: (res) => {
						...
						var url='http://110.fonery.cn:8025';
						var result=res.data.data;
						this.Name=result.Name;
						this.IDCard=result.IDCard;
						this.IDCardPositive=url+result.IDCardPositive;
						this.IDCardNegative=url+result.IDCardNegative;
						this.QRCode=url+result.QRCode;
						this.WeChat=result.WeChat;
						this.Address=result.Address;
						this.BankCardNumber=result.BankCardNumber;
						this.PhoneNumber2=result.PhoneNumber2;
					},
				})
				this.isShow=false;
				this.isClick=false;  //如果有个人信息，就不点击上传
			...
			subInfo(){
				...
				var WeChat=_self.WeChat;
				var Address=_self.Address;
				var PhoneNumber2=_self.PhoneNumber2;
				var BankCardNumber=_self.BankCardNumber;
				var IDCardPositive=_self.IDCardPositive;
				var IDCardNegative=_self.IDCardNegative;
				var QRCode=_self.QRCode;
				if(Name==''){
					uni.showToast({
						title:'姓名不能为空',
						icon:'none'
					...
					uni.request({
						...
						success: (res) => {
							...
							if(res.data.Success==true){
								...
								_self.states=res.data.ReturnString;
								uni.setStorageSync('states',res.data.ReturnString);
								if(_self.states=='已提交'&&userID!=''){
								   _self.showInfo();
								}
							}else{
								uni.showToast({
